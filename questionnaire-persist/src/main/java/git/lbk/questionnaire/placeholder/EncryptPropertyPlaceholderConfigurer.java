...
/**
 * 读取加密的配置文件
 * fixme 这里所谓的加密其实就是使用固定密码把内容加密, 读取的使用使用固定密码读取. 虽然看配置文件的时候心里觉得挺"安全"的.  可是密码是明文写在源码里的! 而且一直没法更换密码, 就算看不到源码, 也能暴力破解. 那么这里的加密该怎么做才好呢? 如果密码不写到源码里, 就没法读取配置文件, 写到文件里又不安全. 就算使用非对称加密把密码加密了, 也得在源码可以读取的地方放置公/私钥吧.
 */
